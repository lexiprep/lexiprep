...
/**
 * DEPRECATED — replaced by {@link wordSenses} (one row per sense). Kept in the schema
 * for one release so `db:push` (which runs *before* `db:migrate` at startup) doesn't
 * drop the data before the copy migration (`drizzle/0003_normalize_definitions.sql`)
 * has run. No code reads or writes it anymore; drop it in a follow-up release.
 */
...
/**
 * Normalized dictionary: **one row per sense** (spec 03, normalized for the AI-definition
 * feature). Two kinds of rows share the table:
 *  - `bookId IS NULL` — shared dictionary senses (Open English WordNet via
 *    `make dict-update`, or the Free Dictionary fallback), global per (language, lemma).
 *  - `bookId` set + `ai = true` — an AI-generated contextual sense for that lemma *in
 *    that book* (source `ai`; the job state lives in {@link aiDefinitions}).
 * `idx` preserves sense order within its (language, lemma, bookId) group.
 */
export const wordSenses = pgTable(
  "word_senses",
  {
    ...
    example: text("example"),
    /** 'oewn-2025' | 'freedict' | 'ai' — provenance. */
    source: text("source").notNull(),
    ai: boolean("ai").notNull().default(false),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
    index("word_senses_lang_lemma_idx").on(t.language, t.lemma),
    index("word_senses_book_lemma_idx").on(t.bookId, t.lemma),
  ],
);

/**
 * Free Dictionary fallback bookkeeping: a row = "we asked the API about this lemma and
 * got a definitive answer" (found, or definitively absent). Replaces the old
 * `definitions` row-with-empty-senses negative cache — in the normalized table an
 * absent word has zero {@link wordSenses} rows, which alone can't be told apart from
 * "never fetched". Transient API errors record nothing, so they retry later. The unique
 * key also serializes concurrent fetches (first insert wins → only it writes senses).
 */
export const definitionFetches = pgTable(
  "definition_fetches",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    language: text("language").notNull(),
  ...
  (t) => [unique("definition_fetches_lang_lemma_uniq").on(t.language, t.lemma)],
);
...
/**
 * A user's own definitions/notes on a word, scoped to one book — the word may carry a
 * specific meaning in that book's context. A word may have **several** rows per
 * (user, book, lemma) — the user can add multiple definitions; in a book-scoped view
 * they replace the AI/dictionary definition. Keyed by the base form (lemma), addressed
 * individually by `id` for edit/delete.
 */
export const wordNotes = pgTable(
  ...
  (t) => [index("word_notes_user_book_lemma_idx").on(t.userId, t.bookId, t.lemma)],
);

/**
 * AI contextual-definition job state, one row per (book, lemma) — spec 10's
 * "strongest paid hook", generated by the `ai-definition` pg-boss queue via OpenRouter.
 * The unique key is the dedupe + never-regenerate backbone: a `done` row is immutable
 * (its senses live in {@link wordSenses} with `ai = true`); only a `failed` row may be
 * flipped back to `pending` (manual retry). Books are per-user, so bookId implies the
 * owner. Model/token/cost columns are the per-use audit trail for a paid feature.
 */
export const AI_DEFINITION_STATUSES = ["pending", "done", "failed"] as const;
export type AiDefinitionStatus = (typeof AI_DEFINITION_STATUSES)[number];

export const aiDefinitions = pgTable(
  "ai_definitions",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    bookId: uuid("book_id")
...
export type NewWordSenseRow = typeof wordSenses.$inferInsert;
export type AiDefinition = typeof aiDefinitions.$inferSelect;
export type NewAiDefinition = typeof aiDefinitions.$inferInsert;
